...
for i in range(100):
    print(f"loss at epoch {1+i}: {history.history['loss'][i]}")
...
# loss at epoch 1: 129.97262573242188
# loss at epoch 100: 9.097864151000977
...
def predict_and_sample(inference_model, x_initializer = x_initializer, a_initializer = a_initializer, 
    ...
    indices = np.array(pred)
    # Step 3: Convert indices to one-hot vectors, the shape of the results should be (Ty, n_values)
    results = to_categorical(x_initializer.shape[2], num_classes=None)
    ### END CODE HERE ###
